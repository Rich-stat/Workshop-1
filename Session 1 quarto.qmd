...
This first workshop aims to introduce you to R and RStudio. R is the programming language; RStudio is the IDE (Integrated Development Environment) that makes R easier to use. <br><br>We are going to use RStudio on the cloud (Posit Cloud) during the workshop sessions. However, the free version only permits 25 hours of computing time per month, and I strongly recommend that you also set up R and RStudio on your own laptop and save the cloud files that we create on the cloud version to your laptop. To use RStudio on your own laptop, you must first download R and then download RStudio (R is a requirement for RStudio to work).<br> You can, of course, also bring your own laptop to the workshops and work on that there. <br><br> The 1st tab below shows you how to set up RStudio on the cloud.<br> The 2nd tab shows you how to set up R and RStudio on your own laptop. <br><br> *NB* You can use RStudio on a university computer from AppsAnywhere, but it is not ideal. the latest versions of R and RStudio are not installed, and you have to install previously used packages every time you use it.
:::
...
- Select the blue icon in the top right corner that says '**New Project**'. <br>Then select **New RStudio Project** and create a new RStudio project called '**LS5022 Biostatistics**'. You can rename the project in the box at the top where it initially says 'Untitled Project' (you are allowed 25 projects on the free plan). <br>It is here that we will work and save our files in all of the workshops. <br>

## Install R & RStudio (Laptop)

##### **1. Install R**
First you need to install R.  Go to the R website ('The Comprehensive R Archive Network') at<br> [R (click on this link)](https://cloud.r-project.org/) <br><br>
The top of the page looks like this:

```{r}
#| label: fig-R_Network_png
#| fig-cap: "R web site"
#| fig-alt: "The Comprehensive R Archive Network page"
#| out.width: "150%"
#| fig-height: 2
#| echo: false
#| column: page-right
#| lightbox: true
knitr::include_graphics("R Download.png")
```

Click on the download link appropriate for your operating system (Windows or Mac) and follow the instructions.
...
##### **2. Install RStudio**
Go to [RStudio (click on this link)](https://posit.co/download/rstudio-desktop/) <br><br> The page looks like this:<br>

```{r}
#| label: fig-RStudio_web_png
#| fig-cap: "RStudio web site"
...
#| column: page-right
#| lightbox: true
knitr::include_graphics("RStudio IDE.png")
```

This is the RStudio IDE User Guide which provides some useful information about RStudio as well as the download links for RStudio.<br><br>
Scroll down to `Direct Downloads (Open Source)`, select the version appropriate for your operating system (Windos or Mac) and follow the instructions.<br>

```{r}
#| label: fig-RStudio_down_png
#| fig-cap: "RStudio download versions"
#| fig-alt: "Select the RStudio version for your laptop"
#| out.width: "150%"
#| fig-height: 3.0
#| echo: false
#| column: page-right
#| lightbox: true
knitr::include_graphics("RStudio IDE Downloads.png")
```

*Note*: R and RStudio update quite regularly, and so it is a good idea to check the latest versions from time to time (RStudio will prompt you if there are new versions released). To update, simply repeat the instructions above to install the latest version of R.
...
- At the bottom right is the the **File Directory** and where you can navigate to your desired folder, create and delete folders and also see installed packages. Use the Files tab to select the folder in which you wish to save your work.
  - Then click on the More cog and select `Set as Working Directory`. This is now your working folder (you will see the code run in the R Console). <br><br>
...
In the next tabs, you will do some simple calculations in R and import an Excel file to R. Most of the work that we will do will involve carrying out statistical analyses on data imported from Excel files.

## Some simple things!

::: {.callout-tip appearance="default" icon="false"}
**TO START THIS NEW SESSION:**
:::

We will be using the project that you have already created in Posit cloud called **'LS5022 Biostatistics'**.

- Create a new folder within the project called 'Session 1' where we will store today's work.

```{r}
#| label: fig-newFolder_png
#| out-width: "75%"
#| fig-height: 3.5
#| fig-cap: "Create a new folder"
#| echo: false
#| lightbox: true
knitr::include_graphics("NewFolder.png")
```

- Next, set this as your working directory.
- Open a new R Script file in the code editor.
- The first thing I want to do in R is to load some packages that enhance R for what we are about to do. R has a base language, but packages significantly enhance and add power to what R can do.<br>
...
These packages will be saved in RStudio to be used in future.<br> *NB* You can always check which packages you have installed by clicking on 'Packages' in the File Directory window.
:::

- Once packages are installed you call them in R by using the `library()` function:<br> *NB* You need to do this every time you start a new session.
...
*NB* If R ever says that a package does not exist, it means you need to install it before loading it with `library()`.<br><br> If you want to check what packages are installed, the File Directory window has a tab called 'Packages'. Installed packages are listed. Packages that have been loaded have a tick by them. <br><br><br>
...
<br> **2. Some basic arithmetic!** <br> Addition: the calculation is stored in an object called 'add'
...
<br> Here, `mean()`, `sd()` and `describe()` are called **functions**. All functions are followed by () where arguments can be placed to specify what action is to be performed. We load particular packages to make use of their functions than enhance R.<br> <br> <br> **6. Save your r file on the cloud in your working directory.**<br> Once saved, you can download it to your own laptop:<br>
...
- Move it to a folder where you are saving your work for this module. <br><br>
...
*NB* if you are using your own laptop, you can simply locate the Downloads folder in the Files directory and move the file to your working directory using the 'More' cog icon and 'Move...' <br>
...
- Now import the xlsx file and store it as a dataframe called *chol_df*.<br>The following code uses the rio package to use the `import()` function.<br>It loads the Excel file and stores the data in a data frame object called '*chol_df*'<br>
...
*NOTE*: this is saved as a `data frame`. This is the type of object that we will be working with.<br> <br> You should now see the chol_df object in the Environment window (top right) in RStudio. <br> You can view it by simply clicking on the object name in the Environment.

<br>**8. Save the chol_df dataframe as an RData file in your desired folder** <br>(*NB* This code will save the file to wherever your current working directory is set).

This means the data frame is saved and you don't have to import the excel file every time you open RStudio! Just type in R (or just copy and paste the script):
